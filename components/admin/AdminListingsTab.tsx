...
const AdminListingsTab: React.FC<AdminListingsTabProps> = ({ 
  ...
  const getDealerName = (dealerId: string) => {
    ...
    return dealer?.name || t('admin.listings.unknownDealer', { defaultValue: 'Unknown dealer' });
  };
  ...
  }, [listings, searchTerm, statusFilter, dealers, t]);
  ...
  const statusLabels: Record<string, string> = {
    active: t('admin.listings.status.active', { defaultValue: 'Active' }),
    pending: t('admin.listings.status.pending', { defaultValue: 'Pending review' }),
    rejected: t('admin.listings.status.rejected', { defaultValue: 'Rejected' }),
    sold: t('admin.listings.status.sold', { defaultValue: 'Sold' }),
    inactive: t('admin.listings.status.inactive', { defaultValue: 'Hidden' }),
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row gap-4 justify-between items-center bg-white/5 p-4 rounded-xl border border-white/10">
        <div className="relative w-full lg:w-96">
          ...
          <input
            type="text"
            autoComplete="off"
            placeholder={t('admin.listings.searchPlaceholder', { defaultValue: 'Search make, model, or dealer...' })}
            value={searchTerm}
        ...
        <div className="flex gap-3 w-full lg:w-auto">
          <div className="relative flex-1 lg:flex-none">
            ...
            >
              <option value="all">{t('admin.listings.allStatuses', { defaultValue: 'All statuses' })}</option>
      ...
      {selectedIds.length > 0 && (
        <div className="flex flex-col gap-3 rounded-xl border border-gray-cyan/30 bg-gray-cyan/10 p-4 animate-in fade-in slide-in-from-top-2 sm:flex-row sm:items-center sm:gap-4">
          ...
          <div className="hidden h-4 w-px bg-gray-cyan/30 sm:block" />
              ...
              <tr>
                <th className="px-6 py-4 w-10">
                  <input
                    ...
                    aria-label={t('admin.listings.selectAll', { defaultValue: 'Select all listings' })}
                    className="rounded border-white/10 bg-white/5 text-gray-cyan focus:ring-gray-cyan/50"
                  />
                </th>
                <th className="px-6 py-4 font-semibold">{t('admin.listings.vehicle', { defaultValue: 'Vehicle' })}</th>
                ...
                <tr>
                  <td colSpan={6} className="px-6 py-12 text-center text-gray-500">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <p>{t('admin.listings.noResults', { defaultValue: 'No listings found matching your criteria.' })}</p>
                    </div>
                  </td>
                </tr>
              ) : (
                  ...
                  <tr key={listing.id} className={`hover:bg-white/5 transition-colors ${selectedIds.includes(listing.id) ? 'bg-gray-cyan/5' : ''}`}>
                    <td className="px-6 py-4">
                      <input
                        ...
                        aria-label={t('admin.listings.selectListing', {
